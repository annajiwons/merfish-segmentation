...
# Number of (training, validation) examples to use
num_train = [(5, 2), (10, 3)]
...
for i in range(len(num_train)):
    ...
    model = StarDist2D(conf, name = f'2D_versatile_fluo_{num}', basedir = model_dir) 
    model.train(merfish_X_manual_train[:num], merfish_Y_manual_train[:num], 
                                    validation_data=(merfish_X_manual_valid[:num_valid], merfish_Y_manual_valid[:num_valid]), 
                                    augmenter=None, epochs=300)
    model.optimize_thresholds(merfish_X_manual_valid[:num_valid], merfish_Y_manual_valid[:num_valid])